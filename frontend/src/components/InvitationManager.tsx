...
const InvitationManager: React.FC<InvitationManagerProps> = ({ siteUrl }) => {
  ...
  const generatePDFDoc = async (guest: GuestData): Promise<jsPDF> => {
    ...
    doc.setFont("times", "normal");
    doc.setFontSize(9);
    doc.text("Cordially Invited:", cx, boxY + 8, {
      align: "center",
    });
    ...
    doc.setFontSize(10);
    doc.text(
      "Request the honour of your presence at the wedding celebration of:",
      cx,
      95,
      { align: "center" }
    );
    ...
    doc.text("With joyful hearts, the celebration will be held on:", cx, 35, {
      align: "center",
    });
    doc.setFont("times", "bold");
    doc.setFontSize(14);
    ...
    doc.text(`Time: ${akadStart} - ${akadEnd}`, cx, 62, {
      align: "center",
    });
    doc.setDrawColor(
      currentTheme.secondary[0],
    ...
    doc.text(`Time: ${resepsiStart} - ${resepsiEnd}`, cx, 97, {
      align: "center",
    });
    doc.setFont("times", "bold");
    doc.setFontSize(12);
    ...
    doc.text("VENUE & LOCATION:", cx, 115, { align: "center" });
    doc.setFont("times", "normal");
    doc.setFontSize(11);
    ...
    doc.text("Scan for Google Maps", cx, qrY + qrSize + 5, { align: "center" });
    ...
    doc.text(
      "Scan to view digital invitation & RSVP",
      cx,
      digiY + qrDigiSize + 8,
      { align: "center" }
    );
  ...
  const downloadTemplate = () => {
    const csvContent =
      "Guest Name,Address (Optional)\nMr. & Mrs. Anderson,New York\nCollege Alumni Group,Honored Guest\nDavid & Family,San Francisco";
    const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
    handleSaveAs(blob, "wedding_invitation_template.csv");
  };
      ...
      Papa.parse(file, {
        complete: (results) => {
          ...
          results.data.forEach((row: any) => {
            const name = row[0];
            const address = row[1] || "Honored Guest";
            if (name && typeof name === "string" && name.trim() !== "") {
              if (!name.toLowerCase().includes("guest name") && !name.toLowerCase().includes("nama tamu")) {
  ...
  const processBulk = async () => {
    ...
    try {
      const zip = new JSZip();
      const folder = zip.folder(`Invitations_${currentTheme.name}_Floral`);
      const CHUNK_SIZE = 10;
      for (let i = 0; i < bulkData.length; i += CHUNK_SIZE) {
        const chunk = bulkData.slice(i, i + CHUNK_SIZE);
        setStatusMsg(
          `Generating PDF ${i + 1} - ${Math.min(i + chunk.length, bulkData.length)} of ${bulkData.length}...`
        );
        await Promise.all(
      ...
      handleSaveAs(
        content,
        `Invitations-${currentTheme.id}-${new Date().toISOString().slice(0, 10)}.zip`
      );
      setStatusMsg("Complete!");
    } catch (error) {
      console.error("Failed to generate bulk:", error);
      alert("A system error occurred while generating PDF.");
    } finally {
      setTimeout(() => {
    ...
    <div className="animate-reveal space-y-8">
      <div className="space-y-3 text-center">
        ...
        <p className="mx-auto max-w-2xl text-sm leading-relaxed text-slate-600 dark:text-slate-400">
          Elegant Floral Template (4-page A5) with Vector Ornaments, QR Codes &
          Multi-Theme Support
        </p>
      </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  placeholder="Example: Mr. & Mrs. Anderson"
          ...
          <div className="flex min-h-[400px] items-center justify-center rounded-xl bg-slate-200 p-4 dark:bg-slate-900/50">
            {previewUri ? (
              ...
              <div className="text-center text-slate-400">
                ...
                <p className="text-sm">Enter guest details & click Preview</p>
                <p className="mt-1 text-xs text-slate-500">
                  (Floral ornaments rendered with vector PDF graphics)
                </p>
              </div>
            )}
          </div>
            ...
            <div className="space-y-6 rounded-3xl border-2 border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-800/50">
              ...
              <div className="space-y-2">
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">
                  {isReadingCsv
                    ? "Reading CSV File..."
                    : "Upload Guest CSV Database"}
                </h3>
                <p className="mx-auto max-w-md text-sm text-slate-500">
                  Prepare a CSV file with format:
                  <br />
                  <strong>Column 1: Guest Name</strong> |{" "}
                  <strong>Column 2: Address/City (Optional)</strong>
                </p>
